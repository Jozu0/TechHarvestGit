    ...
    public void AddItem(Ressource ressourceToAdd)
    {
        ...
        foreach (RessourceItem ressourceItem in ressourceToAdd.ressourceItemList) //foreach imbriqué mais genre max 100 itérations
        {
            ...
            { // Verifie si la ressource existe déjà dans le slot actuel, et si le slot est vide;
                if (ressource.GetComponent<SlotUI>().actualRessourceItem!=null && // si le slot est vide
                    ressource.GetComponent<SlotUI>().actualRessourceItem.ressourceType == ressourceItem.ressourceType) // Si le slot possède le mm ressource type que l'object actuel;
                {
                        ressource.GetComponent<SlotUI>().AddMoreItem(); // On ajoute +n item dans la liste
                        globalRessourceList.AddRessource(ressourceItem.ressourceType,ressourceItem,ressource.GetComponent<SlotUI>().amountToAdd); // Rajoute le nouvel item dans l'inv global;
                        break;
                }
